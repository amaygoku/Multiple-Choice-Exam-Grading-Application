...
export default function ReviewAndEdit({
  ...
  const editorCode = useMemo(
    () => editorExam?.codes.find(code => code.code === selectedResult?.examCode) ?? null,
    [editorExam, selectedResult?.examCode]
  );
  ...
  const activeGrading = useMemo(() => {
    if (!draft || !editorCode) return null;
    return gradeAnswers(draft.answers, editorCode.answerKey);
  }, [draft, editorCode]);
  ...
  const saveDraft = async () => {
    ...
    setSaving(true);
    try {
      const selectedCode = editorExam?.codes.find(code => code.code === draft.examCode) ?? null;
      const matchedStudent = editorClass?.students.find(student => student.mssv === draft.studentMssv) ?? null;
      const grading = selectedCode ? gradeAnswers(draft.answers, selectedCode.answerKey) : activeGrading;
      const totalQuestions = selectedResult.totalQuestions || editorExam?.questionCount || draft.answers.length;
      const answers = Object.fromEntries(draft.answers.map((answer, index) => [index + 1, answer || null]));

      const updated: ScannedResult = {
        ...selectedResult,
        studentId: matchedStudent?.id ?? draft.studentId,
        ...
        totalQuestions,
        answers,
        status: selectedCode ? (matchedStudent ? 'matched' : 'unmatched') : 'unknown_code',
        isEdited: true,
      ...
      {isEditorOpen && selectedResult && draft && (
        <div className="fixed inset-0 z-[120] bg-black/75 backdrop-blur-sm p-0 md:p-8 flex items-stretch md:items-center justify-center">
          <div className="w-full max-w-[1500px] h-[100dvh] md:h-[92vh] overflow-hidden rounded-none md:rounded-[32px] bg-[#101a2b] text-white border-0 md:border md:border-white/10 shadow-2xl flex flex-col">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto md:overflow-hidden p-3 sm:p-4 md:p-6">
              <div className="mb-3 xl:hidden rounded-2xl border border-white/10 bg-black/20 p-2">
              ...
              <div className="h-full grid grid-cols-1 xl:grid-cols-[0.72fr_1.62fr_0.66fr] gap-3 min-w-0">
                <div className={`${editorTab === 'student' ? 'block' : 'hidden'} xl:block space-y-3 min-w-0`}>
                ...
                <div className={`${editorTab === 'crops' ? 'block' : 'hidden'} xl:block space-y-3 min-w-0`}>
                ...
                <div className={`${editorTab === 'answers' ? 'block' : 'hidden'} xl:block rounded-2xl bg-[#0d1524] border border-white/10 p-3 md:p-4 flex flex-col min-h-0 h-full overflow-hidden min-w-0`}>
                  ...
                  <div className="mt-4 flex-1 min-h-0 overflow-y-auto pr-1 space-y-3 max-h-[46vh] sm:max-h-none">
                    {draft.answers.map((value, index) => {
                      const normalized = normalizeAnswerKeyValue(value || '');
                      const correct = editorCode?.answerKey[index] ?? '';
                      return (

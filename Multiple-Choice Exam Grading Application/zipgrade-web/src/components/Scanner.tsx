...
export default function Scanner({ classes, existingResults, selectedClassId, selectedExamId, onSelectContext, onResult, onClose }: ScannerProps) {
  ...
  const selectedExamCode = useMemo(() => {
    ...
    return selectedExam.codes.find(code => code.code === draft.examCode) ?? selectedExam.codes[0] ?? null;
  }, [draft, selectedExam]);
  ...
  const selectedAnswerKey = selectedExamCode?.answerKey ?? selectedExam?.codes[0]?.answerKey ?? [];
  ...
  const saveReview = useCallback(async () => {
    ...
    const studentId = matchedStudent?.id ? Number(matchedStudent.id) : null;
    if (!Number.isFinite(classId) || !Number.isFinite(examId) || (examCodeId != null && !Number.isFinite(examCodeId)) || (studentId != null && !Number.isFinite(studentId))) {
      setError('Academic data is not synced yet. Please wait a moment and try again.');
      return;
    }
    const grading = currentGrading ?? gradeAnswers(draft.answers, selectedExamCode?.answerKey ?? selectedExam.codes[0]?.answerKey ?? []);
    ...
    const duplicateResult = existingResults.find(result => {
      ...
      if (studentId != null && result.studentId != null) return result.studentId === String(studentId);
      return result.studentMssv === detectedMssv || result.studentMssv === draft.mssv.trim();
    });
    ...
    const result: ScannedResult = {
      ...
      studentName: matchedStudent?.fullName || draft.name || 'Unknown student',
      detectedName: draft.name || '',
      ...
      scannedAt: Date.now(),
      status: !selectedExamCode ? 'unknown_code' : matchedStudent ? 'matched' : 'unmatched',
    };
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/submissions`, {
        ...
        body: JSON.stringify({
          ...
          student_info: draft.backend.student_info ?? null,
          grading,
          crops: draft.backend.crops ?? null,
    ...
    onResult(result);
    setStatus('ready');
  }, [draft, selectedClass, selectedExam, selectedExamCode, currentGrading, onResult, existingResults]);
        ...
        <AnimatePresence mode="wait">
          {isReviewMode ? (
            ...
            <div className="text-sm text-slate-400 mb-1">Student ID (OMR)</div>
                        <input
                          value={draft.mssv}
                          onChange={(e) => setDraft({ ...draft, mssv: e.target.value })}
            ...
            <div className="text-sm font-black text-white mt-1">{selectedExamCode ? 'Code matched' : 'No code match'}</div>
                      </div>
                ...
                <div className="xl:max-h-[calc(100dvh-7.5rem)] xl:min-h-0">
                <StageCard subtitle="OMR results" title={`${totalQuestions} questions`}>
                  <div className="flex flex-col h-full min-h-0">
                    ...
                    <div className="pt-4 border-t border-white/10 bg-black/20 flex flex-col gap-3">
                      <button
                        onClick={saveReview}
                        className="w-full py-4 rounded-2xl bg-gradient-to-r from-indigo-500 to-violet-500 text-white font-black text-lg shadow-lg shadow-indigo-900/30"
                      >
                        Save reviewed result
                      </button>
                      <button
                        onClick={resetReview}
                        className="w-full py-3 rounded-2xl border border-white/10 bg-white/5 text-white font-bold"
                      >
                        Scan another paper
                      </button>
                    </div>
          ...
          <label className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">Class</label>
                      <select
                        ...
                        className="w-full min-h-12 bg-white/5 border border-white/10 rounded-xl px-3 py-3 text-sm font-bold"
                      >

...
export default function Analytics({ classes, results, onStartScanner }: AnalyticsProps) {
  ...
  const summaryFallback = useMemo<SummaryView>(() => {
    const scores = filteredStudentResults.map(item => item.score);
    return {
      ...
      passCount: filteredStudentResults.filter(item => item.score >= 5).length,
      excellentCount: filteredStudentResults.filter(item => item.score >= 8).length,
      matchedCount: filteredStudentResults.filter(item => item.status === 'matched').length,
    };
  ...
  const scoreDistribution = useMemo<DistributionView[]>(() => {
    ...
    return buildLocalDistribution(filteredStudentResults.map(item => item.score));
  }, [filteredStudentResults, remoteAnalytics]);
  ...
  const gradeBands = useMemo<GradeBandView[]>(() => {
    if (remoteAnalytics) {
      ...
      return remoteAnalytics.grade_bands.map((band, index) => ({
        label: band.label,
        value: band.count,
        percent: percent(band.count, summary.count),
        color: colors[index] ?? 'bg-slate-500',
      }));
    }
    return buildLocalGradeBands(filteredStudentResults);
  }, [filteredStudentResults, remoteAnalytics, summary.count]);
  ...
  const comparisonItems = useMemo<ComparisonView[]>(() => {
    ...
    return buildLocalComparison(classes, filteredStudentResults, selectedClassId, selectedExamId, codeOptions);
  }, [classes, codeOptions, filteredStudentResults, remoteAnalytics, selectedClassId, selectedExamId]);
  ...
  const questionAnalysis = useMemo<QuestionView[]>(() => {
    ...
    return buildLocalQuestions(filteredStudentResults, selectedAnswerKey);
  }, [filteredStudentResults, remoteAnalytics, selectedAnswerKey]);
  ...
  const studentScoreRows = useMemo(() => {
    return filteredStudentResults
      .map(result => ({
        id: result.id,
        studentName: result.studentName?.trim() || result.detectedName?.trim() || 'Unknown student',
        studentId: result.studentMssv?.trim() || '-',
        examCode: result.examCode?.trim() || '-',
        score: result.score,
      ...
      {filteredResults.length === 0 ? (
        ...
        <>
          <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            <StatCard icon={<ClipboardList size={18} />} label="Total students" value={`${summary.count}`} helper="Unique students in scope" />
            <StatCard icon={<TrendingUp size={18} />} label="Average score" value={`${formatScore(summary.avg)}/10`} helper={`Median ${formatScore(summary.med)}`} />
            ...
            <StatCard icon={<Users size={18} />} label="Pass rate" value={`${percent(summary.passCount, summary.count)}`} helper={`Excellent: ${percent(summary.excellentCount, summary.count)}`} />
          </section>
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <div className="text-sm text-slate-600 mt-1">
                            Difficulty p = {item.difficulty.toFixed(2)} · Blank {percent(item.blankCount, summary.count)} · Trap answer: <span className="font-black text-slate-900">{item.trapOption}</span>
                          </div>
                        </div>
                      ...
                      <div className="max-h-[420px] overflow-y-auto divide-y divide-white/5">
                        {studentScoreRows.map((row) => (
                          ...
                          >
                            ...
                            <div className="text-right text-sm font-black tabular-nums text-cyan-200">{formatScore(row.score)}</div>
                          </div>
                        ))}
                      </div>
                    </div>

...
def _find_existing_submission(db: Session, payload: SubmissionCreate) -> Submission | None:
    ...
    if payload.student_id is not None:
        return query.filter(Submission.student_id == payload.student_id).order_by(Submission.id.desc()).first()
    if payload.detected_mssv:
        return query.filter(Submission.detected_mssv == payload.detected_mssv).order_by(Submission.id.desc()).first()
    return None
...
def create_submission(payload: SubmissionCreate, db: Session = Depends(get_db)):
    ...
    existing = _find_existing_submission(db, payload)
...
def get_analytics(
    ...
    submissions = query.order_by(Submission.scanned_at.desc(), Submission.id.desc()).all()
    ...
    scores = [float(item.score) for item in student_results]
    ...
    summary = AnalyticsSummary(
        ...
        pass_count=sum(1 for item in student_results if float(item.score) >= 5.0),
        pass_rate=_percent(sum(1 for item in student_results if float(item.score) >= 5.0), total),
        excellent_count=sum(1 for item in student_results if float(item.score) >= 8.0),
        excellent_rate=_percent(sum(1 for item in student_results if float(item.score) >= 8.0), total),
    ...
    grade_bands = _build_grade_bands(total, scores)
    comparison = _build_comparison(student_results, classes, class_id, exam_id)
    ...
    top_failed_questions = _build_question_insights(student_results, target_answer_key)[:10] if target_answer_key else []
